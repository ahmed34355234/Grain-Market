...
const Roznamcha = () => {
	...
	useEffect(() => {
		const fetchItems = async () => {
			...
			try {
				const data = await Addtransactions.getTransactionItems(user.id)
	...
	useEffect(() => {
		const fetchWarehouses = async () => {
			...
			try {
				...
				// Normalize the response into an array of strings or objects
				let warehouseList = []
				...
				if (response) {
					if (Array.isArray(response)) {
						...
						warehouseList = response.warehouses
					} else if (typeof response === 'object') {
						// Maybe it's { warehouse: { name: "..." } } – extract names
						warehouseList = Object.values(response).filter(
							(item) =>
				...
				// Final fallback: ensure it's array of strings/names
				const normalized = warehouseList
					.map((item) =>
				...
				setWarehouses(Array.isArray(fallback) ? fallback : [])
			}
		}

		fetchWarehouses()
	}, []) // Run once, or depend on user login change if needed
